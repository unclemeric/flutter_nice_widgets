...
import 'package:flutter_nice_widgets/widgets/numberKeyboard.dart';
...
class PasswordInput extends StatefulWidget {
  ...
  PasswordInput({Key? key, this.value: "", this.length: 6, this.mask: true, this.hideWhenSubmitted: false, this.info, this.onClick, this.onChange, this.onSubmitted}) : super(key: key);
    ...
    return Expanded(
      child: Container(
          ...
          margin: EdgeInsets.only(
            ...
            right: widget.length < 6 && p < widget.length - 1 ? Style.passwordInputGutter : 0,
          ),
          decoration: BoxDecoration(
              borderRadius: widget.length < 6
                  ? BorderRadius.circular(Style.passwordInputBorderRadius)
                  : p == 0
                      ? BorderRadius.horizontal(left: Radius.circular(Style.passwordInputBorderRadius))
                      : p == widget.length - 1
                          ? BorderRadius.horizontal(right: Radius.circular(Style.passwordInputBorderRadius))
                          : null,
              border: (p == 0 || p == widget.length - 1)
                  ? Border.all(color: Style.borderColor)
                  : Border(
                      top: BorderSide(color: Style.borderColor, width: Style.borderWidthBase),
                      left: BorderSide(color: Style.borderColor, width: widget.length < 6 ? Style.borderWidthBase : 0),
                      right: BorderSide(color: Style.borderColor, width: p < widget.length - 2 || widget.length < 6 ? Style.borderWidthBase : 0),
                      bottom: BorderSide(color: Style.borderColor, width: Style.borderWidthBase),
                    ),
          ...
          child: Text(
              _codeList.elementAt(p) == ''
                  ...
                  : widget.mask
                      ? "●"
                      : _codeList[p],
              style: TextStyle(fontSize: Style.passwordInputFontSize, color: Style.passwordInputColor))),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        ...
        widget.info != null ? Text(widget.info!, style: TextStyle(fontSize: Style.passwordInputInfoFontSize, color: Style.passwordInputInfoColor)) : Container()
      ],
    );
  }
}
